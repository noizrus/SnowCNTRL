...
/// Free, keyless 7-day forecast from Open-Meteo — no API key or paid
...
/// pick this API over a plainer one: knowing more snow is coming this week
/// is directly useful next to a snow-clearing ban tracker.
enum WeatherService {
    ...
    private static func parse(_ daily: Response.Daily) -> [DailyForecast]? {
        ...
        return (0..<count).compactMap { i in
            ...
            let parts = daily.time[i].split(separator: "-").compactMap { Int($0) }
            guard parts.count == 3,
